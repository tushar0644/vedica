...
const BACKEND_HOST = new URL(BACKEND).hostname   // 'vedica.origensystems.com'
...
function buildHeaders(sid: string, csrf?: string): Record<string, string> {
  ...
  const h: Record<string, string> = {
    Cookie: cookieStr,
    Host: BACKEND_HOST,          // Frappe uses Host header to identify the site
...
export async function POST(
  ...
  // Convert incoming FormData to URL-encoded body (Frappe accepts both)
  ...
  try {
    ...
    // Capture session cookies set by Frappe on login
