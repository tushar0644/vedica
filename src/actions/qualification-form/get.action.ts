...
export const getQualificationForm = async () => {
  try {
    ...
    return {
      data,
      success: true,
      message: response.data.message || "Qualification form data",
    };
  } catch (err: any) {
    console.error("[QUALIFICATION FORM][GET][ERROR]", err.message);

    return {
      success: false,
      message:
        err?.response?.data?.message ||
        "Failed to request qualification form doctype",
    };
  }
};
